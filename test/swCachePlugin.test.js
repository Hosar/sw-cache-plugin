...
import TemplateCreatorW4 from '../src/TemplateCreatorW4';
...
test('given a webpack4 compiler, should return webpack4 template creator', t => {
  const config = webpackConfig();
  const compiler = webpack(config);
  const plugin = new SwCachePlugin(pluginOptions);
  const templateCreator = plugin.getTemplateCreator(compiler);
  t.truthy(templateCreator instanceof TemplateCreatorW4);
});
